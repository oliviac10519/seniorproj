...
#            file=os.path.join(app.config['UPLOAD_FOLDER'], 'Clean_Sorted.xlsx')
#            dftf1.to_excel(file, index=False)
...
#        return render_template('results.html', results=pivots_won, charts=charts, sum_won1=sum_won1, download_excel='Clean_Sorted.xlsx')
...
def analyze1(filename):
    ...
    try:
        ...
        if timeframe1_1 and timeframe1_2:
            ...
            file = os.path.join(app.config['UPLOAD_FOLDER'], 'Clean_Sorted.xlsx')
            dftf1.to_excel(file, index=False)
            app.logger.debug(f"Excel file saved: {file}")
        else:
            flash('Timeframe inputs required')
            return redirect(url_for('one_route', filename=filename))
        ...
        return render_template(
            ...
            charts=charts,
            sum_won1=sum_won1,
            download_excel='Clean_Sorted.xlsx'
        )
...
def analyze(filename):
   ...
   try:
       ...
       if timeframe1_1 and timeframe1_2 and timeframe2_1 and timeframe2_2:
           ...
           file=os.path.join(app.config['UPLOAD_FOLDER'],'Clean_Sorted1.xlsx')
        #    file2=os.path.join(app.config['UPLOAD_FOLDER'],'Clean_Sorted2.xlsxl')
           frames = [dftf1, dftf2]
           clean_timeframes = pd.concat(frames)
           clean_timeframes.to_excel(file, index=False)
           #dftf2.to_excel(file2, index=False)
          
       win_stage = request.form.get('win_stage', type=int)
       lose_stage = request.form.get('lose_stage', type=int)

       pivots_won, charts, sum_won1 = compare_two(dftf1, dftf2, win_stage, lose_stage)
       return render_template('results.html', results=pivots_won, charts=charts, sum_won1=sum_won1, download_excel='Clean_Sorted1.xlsx')
